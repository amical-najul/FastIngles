...
export const SettingsScreen: React.FC<SettingsScreenProps> = ({ user, darkMode, onBack, onUpdateUser, onToggleDarkMode }) => {
    ...
    const handleSaveProfile = async (e: React.FormEvent) => {
        ...
        setMessage(null);
        try {
            // authService.updateProfile in new logic calls backend sync
            // We might want to use context.updateProfile instead if we added it there
            // checking authService implementation... it delegates to apiService.updateUserProfile
            // This seems correct for backend update.
            const updates: any = { name, email, photoUrl };
            // Password update might need special handling with Firebase if we want to support it, 
            // but for now let's assume it updates backend hash or we might need to use Firebase SDK for password change.
            // For simplicity, let's allow backend update request but note that Firebase Auth password is separate.
            // Ideally we use `updatePassword(user, newPassword)` from firebase/auth.

            // For this iteration, we focus on profile data.
            if (password) {
                // TODO: Implement Firebase password update
                setMessage("Nota: El cambio de contraseña debe hacerse vía 'Olvidé mi contraseña' por seguridad en esta versión.");
                return;
            }

            // Use authService (backend sync)
            // Since we are using JIT, updating backend is good.
            // But we ALSO need to update Firebase Profile for name/photo.
            // Let's use the context updateProfile if possible or generic approach.
            // To be safe, let's keep using authService.updateProfile which now maps to backend update
            // AND we should probably update local context.
            // Re-reading implementation plan: "Any profile update MUST be synchronized to BOTH".
            // Let's rely on apiService/authService to handle backend. 
            // We should really update Firebase too.

            const updatedUser = await authService.updateBackendProfile("", updates); // Token handled by interceptor
